...
The configuration is intentionally small. All fields are optional — the instance name is derived from the `instances` map key. When using a config file, omitted fields are left unset; no defaults are injected. Defaults only apply in the interactive wizard flow.

The top-level config is an `instances` map, where each key is the instance name:
...
| `scripts`     | Shell commands to run inside the instance after creation and file transfers | see [Scripts](#scripts)             |
